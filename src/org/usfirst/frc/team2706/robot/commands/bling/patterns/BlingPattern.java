...
public abstract class BlingPattern {
    ...
    /**
     * True if the pattern has already been run at least once and it was the last pattern to run.
     */
    ...
    /**
     * Analyzes whether or not the conditions for this pattern are being met, and returns a boolean
     * based on this. This should be overriden for real bling commands, as by default it returns
     * false every time.
     * 
     * @return True if this bling pattern's operating conditions are met, false otherwise.
     */
    ...
    /**
     * Gets the string command type for the pattern. Should be one of the Bling subsystem's
     * constants
     * 
     * @return
     */
     ...
     * 
     * @return A list of BlingController periods when this pattern is supposed to run, one of
     *         Teleop, Autonomous or Climb or a combination
     */
    ...
    /**
     * Called by bling every time the command is run, which would be about every 20 miliseconds. If
     * the command would like to run something while it is being displayed, it should be run here.
     */
    ...
    /**
     * Should be called when the pattern is ended. Resets the pattern so it can be run again.
     */
    public void end() {
        hasRun = false;
    }
